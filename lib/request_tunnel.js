...
exports.createPersistentClient = function(tunnelServerHost, port, readyCallback) {
  ...
  t.interval = setInterval(function() {
    ...
    if (port == 80)
    {
        var url = 'http://'+tunnelServerHost+'/';
    }
    else
    {
        var url = 'http://'+tunnelServerHost+':'+port+'/';
    }
    var req = request.get(url); // In the future, we will use the url to determine whether to use long polling or not
    ...
    req.on('end', function(d) {
      ...
      {
        console.log('stream endpoint err ', er);
      }
    ...
    try {
      ...
      req.on('end', function(d) {
          try {
              ...
              console.log('req end err',err);
          }
      });

    } catch(err) {
      console.log('req handlers err ',err);
    }
  });
